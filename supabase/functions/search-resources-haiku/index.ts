// ============================================================================
// SEARCH RESOURCES WITH HAIKU 4.5 WEB SEARCH
// ============================================================================
// Alternative search method using Claude Haiku 4.5's native web search
...
// 3. If cache miss: Make ONE web search call with Claude Haiku 4.5
...
/**
 * Call Claude Haiku 4.5 with web search tool to find educational videos
 * Uses site:youtube.com to force YouTube-only results
 */
async function searchWithHaiku(
  topic: string,
  description: string,
  ...
  console.log(`[search-resources-haiku] Simplified topic: ${simplifiedTopic}`);
  console.log(`[search-resources-haiku] YouTube search query: ${youtubeQuery}`);

  // System prompt focused on extracting YouTube URLs from search results
  const systemPrompt = `You search for YouTube educational videos and return them as JSON.

CRITICAL: Use "site:youtube.com" in your search to ONLY get YouTube results.

When you find YouTube videos, return ONLY a JSON array:
...
Return ONLY a JSON array with 3 YouTube video objects.`;

  console.log('[search-resources-haiku] Calling Haiku 4.5 with web search...');
  console.log(`  - Topic: ${topic}`);
  console.log(`  - Simplified: ${simplifiedTopic}`);
  console.log(`  - YouTube query: ${youtubeQuery}`);

  // Call Claude Haiku 4.5 with web search tool enabled
      ...
      tools: [
        {
          ...
          name: 'web_search',
          max_uses: 2, // Allow 2 searches: one initial, one fallback if needed
        }
      ],
      messages: [
  ...
  if (!response.ok) {
    ...
    console.error('[search-resources-haiku] Claude API error:', response.status, errorText);
    throw new Error(`Claude API error (${response.status}): ${errorText}`);
  }

  const data = await response.json();
  console.log('[search-resources-haiku] Claude response received');
  console.log('[search-resources-haiku] Response content blocks:', data.content?.length || 0);
  
  // Log each content block for debugging
  if (data.content) {
    for (let i = 0; i < data.content.length; i++) {
      const block = data.content[i];
      console.log(`[search-resources-haiku] Block ${i}: type=${block.type}`);
      if (block.type === 'text') {
        console.log(`[search-resources-haiku] Block ${i} text preview:`, block.text?.substring(0, 200));
      }
    }
  }
  ...
  // =========================================================================
  // STEP 1: Extract YouTube URLs from web_search_tool_result blocks
  // =========================================================================
  // The web search returns results in these blocks - we need to extract YouTube URLs
  const webSearchBlocks = data.content?.filter((block: any) => 
    block.type === 'web_search_tool_result' || block.type === 'tool_result'
  ) || [];
  
  for (const searchBlock of webSearchBlocks) {
    console.log('[search-resources-haiku] Processing web search result block');
    
    // Web search results can be in different formats
    const searchResults = searchBlock.content || searchBlock.search_results || [];
        ...
        if (url && (url.includes('youtube.com/watch') || url.includes('youtu.be/'))) {
          if (!results.find(r => r.url === url) && results.length < MAX_SEARCH_RESULTS) {
            ...
            results.push({
              ...
              description: result.snippet || result.description || `Educational video about ${topic}`,
              platform: 'YouTube',
              channel_name: undefined,
              channel_url: undefined,
              thumbnail_url: thumbnail,
              duration_seconds: undefined,
              topic_signature: `Educational video about ${topic}: ${result.title || ''}. ${result.snippet || ''}`,
              concepts_covered: [topic],
              difficulty_level: 'intermediate',
              quality_score: 0.75,
              from_cache: false,
            });
            console.log(`[search-resources-haiku] Extracted YouTube URL from search results: ${url}`);
          }
        }
      }
    }
    
    // Also check if it's a string that might contain URLs
    if (typeof searchBlock.content === 'string') {
      const urlMatches = searchBlock.content.match(/https?:\/\/(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)[^\s\)\"<>]+/g);
      if (urlMatches) {
        for (const url of urlMatches) {
          if (!results.find(r => r.url === url) && results.length < MAX_SEARCH_RESULTS) {
            ...
            console.log(`[search-resources-haiku] Extracted YouTube URL from search content string: ${url}`);
          }
        }
      }
    }
  ...
  console.log(`[search-resources-haiku] After web search extraction: ${results.length} YouTube URLs`);
  
  // =========================================================================
  // STEP 2: Extract from text blocks containing JSON (Claude's formatted response)
  // =========================================================================
  const textBlocks = data.content?.filter((block: any) => block.type === 'text') || [];
  
  for (const textBlock of textBlocks) {
    let text = textBlock.text || '';
    console.log('[search-resources-haiku] Processing text block, length:', text.length);
    
    // Try to extract JSON from code blocks first (```json ... ```)
    const codeBlockMatch = text.match(/```(?:json)?\s*([\s\S]*?)```/);
    if (codeBlockMatch) {
      text = codeBlockMatch[1].trim();
      console.log('[search-resources-haiku] Found JSON code block');
    }
    
    // Try to find JSON array in the text
    const jsonMatch = text.match(/\[[\s\S]*\]/);
    if (jsonMatch) {
      console.log('[search-resources-haiku] Found JSON array match');
      try {
        const parsed = JSON.parse(jsonMatch[0]);
        console.log('[search-resources-haiku] Successfully parsed JSON, items:', parsed.length);
        ...
        console.log('[search-resources-haiku] Failed to parse JSON:', e);
      }
    }
    ...
    if (urlMatches && urlMatches.length > 0) {
      console.log('[search-resources-haiku] Found YouTube URLs via regex:', urlMatches.length);
      for (const url of urlMatches) {
        if (!results.find(r => r.url === url) && results.length < MAX_SEARCH_RESULTS) {
          const videoId = extractVideoId(url);
          const thumbnail = videoId 
  ...
  console.log('[search-resources-haiku] Extracted resources:', results.length);
...
async function generateResourceExplanations(
  ...
  console.log(`[search-resources-haiku] Generating explanations for ${resources.length} resources...`);
...
Output valid JSON only, no markdown.`;
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('[search-resources-haiku] Claude explanation API error:', response.status, errorText);
    ...
    if (jsonMatch) {
      try {
        ...
        console.log(`[search-resources-haiku] Parsed ${explanations.length} resource explanations`);
      } catch (parseError) {
        console.error('[search-resources-haiku] Failed to parse explanation JSON:', parseError);
      }
    }
    ...
    for (const resource of resources) {
      ...
      if (matchingExplanation) {
        ...
        if (isExplicitlyIrrelevant) {
          console.log(`[search-resources-haiku] Filtering out explicitly irrelevant resource: ${resource.title}`);
          filteredCount++;
    ...
    if (filteredCount > 0) {
      console.log(`[search-resources-haiku] Filtered out ${filteredCount} explicitly irrelevant resource(s)`);
    }
    console.log(`[search-resources-haiku] ${relevantResources.length} relevant resources remaining`);
    
    return relevantResources;

  } catch (error) {
    console.error('[search-resources-haiku] Error generating resource explanations:', error);
    return resources; // Return resources without explanations on error
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[search-resources-haiku] Starting Haiku web search');
    ...
    console.log('[search-resources-haiku] Generating query embedding for cache lookup...');
    ...
    if (cachedResources && cachedResources.length >= 1) {
      console.log(`[search-resources-haiku] Cache HIT! Found ${cachedResources.length} resources`);
      cacheHit = true;
      ...
      // =========================================================================
      // STEP 3: Cache miss - Search with Haiku 4.5
      // =========================================================================
      
      console.log('[search-resources-haiku] Cache MISS - searching with Haiku 4.5...');
      
      results = await searchWithHaiku(
        topic,
        description || '',
        learning_objective,
        search_queries || []
    ...
    if (results.length === 0) {
      console.log('[search-resources-haiku] No results found');
      ...
      return new Response(
        JSON.stringify({
          ...
          search_metadata: {
            ...
            search_method: 'haiku_web_search',
            total_api_results: 0,
          },
          message: 'No educational videos found for this topic. Try adjusting your search terms or use the YouTube API search.',
        }),
        {
    ...
    if (!cacheHit && results.length > 0) {
      console.log('[search-resources-haiku] Processing new resources with transcript analysis...');
      ...
      for (const resource of results) {
        try {
          ...
          if (existingResource?.transcript_analyzed && existingResource?.content_analysis) {
            ...
            console.log(`[search-resources-haiku] Using cached analysis for: ${resource.title}`);
          ...
          // =========================================================
          console.log(`[search-resources-haiku] Analyzing new resource: ${resource.title}`);
          ...
          if (resource.platform === 'YouTube') {
            ...
            if (videoId) {
              console.log(`[search-resources-haiku] Fetching transcript for video: ${videoId}`);
              const transcriptResult = await fetchTranscript(videoId);
              
              if (transcriptResult.success && transcriptResult.transcript) {
                transcriptText = truncateTranscript(transcriptResult.transcript, 15000);
                transcriptSource = transcriptResult.source;
                console.log(`[search-resources-haiku] Got transcript: ${transcriptResult.wordCount} words (${transcriptSource})`);
                ...
                if (analysisResult.success && analysisResult.analysis) {
                  ...
                  console.log(`[search-resources-haiku] Transcript analysis complete: ${contentAnalysis.concepts_taught.length} concepts`);
                }
              } else {
                console.log(`[search-resources-haiku] No transcript available: ${transcriptResult.error}`);
              }
            }
          }
          ...
          // =========================================================
          if (!contentAnalysis) {
            console.log(`[search-resources-haiku] Using metadata-only analysis for: ${resource.title}`);
          ...
          if (insertError) {
            console.error('[search-resources-haiku] Error storing resource:', insertError);
            console.error('[search-resources-haiku] Insert error details:', JSON.stringify(insertError));
            ...
            if (existingRes) {
              ...
              console.log(`[search-resources-haiku] Found existing resource: ${resource.id}`);
            } else {
              console.error('[search-resources-haiku] Could not find or create resource for URL:', resource.url);
            }
          } else {
            resource.id = insertedResource?.id;
            console.log(`[search-resources-haiku] Stored analyzed resource: ${resource.title}, ID: ${resource.id}, confidence: ${analysisConfidence}`);
          }
          ...
          console.error('[search-resources-haiku] Error in resource storage flow:', storeError);
        }
      }
    }
    ...
    console.log(`[search-resources-haiku] Linking ${results.length} resources to blueprint...`);
    ...
    for (const resource of results) {
      if (resource.id) {
        ...
        if (linkError) {
          console.error('[search-resources-haiku] Error linking resource:', linkError);
        } else {
          console.log(`[search-resources-haiku] Successfully linked resource: ${resource.id}`);
        }
      }
    }
    ...
    if (results.length > 0) {
      console.log(`[search-resources-haiku] Generating explanations for ${results.length} resources...`);
      ...
      console.log(`[search-resources-haiku] After explanation generation: ${afterCount} resources (filtered ${beforeCount - afterCount})`);
      
      // If ALL resources were filtered out, that's a problem - return the original results
      if (afterCount === 0 && beforeCount > 0) {
        console.error('[search-resources-haiku] ⚠️ ALL resources were filtered as irrelevant! This is likely an error.');
        console.error('[search-resources-haiku] Explanation generation may have failed. Returning resources without explanations.');
        ...
        console.log('[search-resources-haiku] Updating junction table with resource explanations...');
        for (const resource of results) {
    ...
    console.log('[search-resources-haiku] Complete!');
    console.log(`  - Results: ${results.length}`);
    console.log(`  - From cache: ${cacheHit}`);
    console.log(`  - Search method: ${cacheHit ? 'cache' : 'haiku_web_search'}`);
    ...
    return new Response(
      JSON.stringify({
        ...
        search_metadata: {
          queries_used: [search_queries?.[0]?.query || topic],
          search_method: cacheHit ? 'cache' : 'haiku_web_search',
    ...
    console.error('[search-resources-haiku] Error:', error);
