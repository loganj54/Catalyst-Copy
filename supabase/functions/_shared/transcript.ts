...
export async function fetchTranscriptsBatch(videoIdsOrUrls: string[]): Promise<BatchTranscriptResult> {
  ...
  try {
    ...
    for (const result of results) {
      ...
      // Check for error or missing transcript
      // Per docs: result.transcript.content contains the transcript segments
      if (result.errorCode || !result.transcript || !result.transcript.content) {
        failed++;
        transcripts.set(videoId, {
          ...
          language: null,
          wordCount: 0,
          error: result.errorCode || 'No transcript content'
        });
        continue;
      }

      // Parse the transcript content from result.transcript.content (array of segments)
      const transcriptText = parseTranscriptData(result.transcript.content);
      ...
      const cleanedText = transcriptText.trim().replace(/\s+/g, ' ');
      const wordCount = cleanedText.split(/\s+/).length;
      // Auto-generated detection from language info if available
      const transcriptLang = result.transcript.lang || 'en';

      succeeded++;
      transcripts.set(videoId, {
        success: true,
        transcript: cleanedText,
        source: 'auto_generated', // Batch API doesn't distinguish
