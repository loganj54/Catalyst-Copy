...
const Blueprint = () => {
  ...
  // Default Tab Logic
  useEffect(() => {
    if (!activeTab && !loading && learningStructure) {
      const struct = learningStructure.structure_data || learningStructure.structure;
      const contentSections = struct?.content_sections;

      if (contentSections && contentSections.length > 0) {
        setActiveTab(contentSections[0].section_id || 'section-0');
      } else if (struct?.prerequisites_section) {
        setActiveTab('prerequisites');
      }
    }
  }, [activeTab, loading, learningStructure, setSearchParams]);
  ...
  if (activeTab === 'chat') {
    currentSectionTitle = 'AI Assistant';
    // Blank canvas: currentUnits remains empty
  ...
  return (
    <div className="flex h-full w-full py-3 pr-3 pl-0 gap-0 overflow-hidden transition-all duration-300 bg-stone-100 dark:bg-stone-950">
              ...
              {tabs.length > 0 && (
                <div className="pt-2">
                  <div className="px-2 mb-2">
                    <h3 className="text-xs font-medium text-stone-500 uppercase tracking-wider">Blueprint Sections</h3>
                  </div>
      ...
      <div className={`flex-1 flex flex-col min-w-0 ml-3 rounded-2xl shadow-xl ring-1 ring-black/5 overflow-hidden relative bg-white dark:bg-stone-950`}>
        ...
        {/* Header - Moved outside scroll container for correct z-index layering */}
        <div className="relative z-30 w-full bg-white dark:bg-stone-900 border-b border-stone-200 dark:border-stone-800 transition-all duration-300">
          <div className="px-6 py-5 relative">
              ...
              <div className="flex items-center gap-2 shrink-0 pointer-events-auto">
                ...
                {(activeTab === 'chat' || activeTab === 'practice-problems-chat') && activeThreadId && (
                  <>
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-1.5 border rounded-lg text-xs font-medium transition-all shadow-sm active:scale-95 ${showChatHistory
                      ...
                      title={showChatHistory ? "Close History" : "View History"}
                    >
                      {showChatHistory ? <X className="w-3.5 h-3.5" /> : <Folder className="w-3.5 h-3.5" />}
                      <span className="hidden sm:inline">{showChatHistory ? 'Close' : 'History'}</span>
                    </button>
                    <button
                ...
                {(doc || blueprint.url || content.text) && (
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-950 border border-transparent hover:bg-stone-800 dark:hover:bg-stone-200 rounded-lg text-xs font-medium transition-all shadow-sm active:scale-95"
            ...
            {/* Horizontal Tabs - REMOVED MOVED TO SIDEBAR */}
          </div>
        </div>
