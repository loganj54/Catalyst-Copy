...
import {
  ...
  MessageSquare, History, Plus, RefreshCw, Play, Star, ExternalLink, Zap, Search, Eye, Database, Calculator, ArrowUpRight, Youtube, Folder, X, Library
} from 'lucide-react';
...
const LectureBlueprintSkeleton = () => {
  ...
  const activeTab = searchParams.get('tab') || 'prerequisites';
  ...
  const fetchBlueprintData = async () => {
    // Prevent fetching if no auth
    if (!session?.access_token || !id) return;

    // Only set loading if we don't have data yet
    // This prevents the "flash" when switching tabs/windows
    if (!blueprint) {
      setLoading(true);
    }
    ...
    try {
      ...
      if (structure?.structure?.structure_type === 'lecture') {
        ...
        // Default to prerequisites tab first ONLY if no tab is specified in URL
      ...
      <div className="flex-1 flex flex-col min-w-0 ml-3 rounded-2xl shadow-xl ring-1 ring-black/5 overflow-hidden relative bg-white dark:bg-stone-950">
        ...
        <div className="relative z-30 w-full bg-white dark:bg-stone-900 border-b border-stone-200 dark:border-stone-800 transition-all duration-300">
          <div className="px-6 py-5 relative">
            {/* View Document Button moved to Right: Actions for consistent height */}
